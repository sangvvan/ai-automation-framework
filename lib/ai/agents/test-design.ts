...
export async function designScenarios(
  ...
  for (const technique of requested) {
    ...
    try {
      ...
      console.warn(`[test-design] AI generation failed for technique "${technique}":`, (err as Error).message);
  ...
  if (out.length === 0 && errors.length > 0) {
    const message = errors.map((e) => `[${e.technique}] ${e.error.message}`).join("; ");
    throw new Error(`AI scenario generation failed completely. Errors: ${message}`);
  }
